...
## Phase 8 — 名前付きレイアウト（v1.1 旗艦・差別化機能）
**狙い**: 複数アプリのウインドウ配置を「レイアウト」として保存し、キーボード1操作で一括復元する（Moom 的だが**キーボード中心**）。Rectangle 無料版にも macOS 標準タイルにも無く、Apple が当面入れない領域＝**陳腐化しない差別化**。
段階的に（各段で手動検証）:
- 8a: 現在の全可視ウインドウ（アプリ名 + frame、対象スクリーン）を `Core/LayoutStore.swift`（純データ + JSON 永続化）にスナップショット保存。
- 8b: 保存レイアウトの一覧 UI（設定に「Layouts」タブ）＋ 命名・削除。
- 8c: 復元 = アプリを bundleID で照合し、各ウインドウへ frame を適用（`position→size→position`、`ScreenGeometry` 経由）。起動していないアプリの扱い（任意で起動 or スキップ）を決める。
- 8d: レイアウトごとにグローバルショートカット割当（KeyboardShortcuts の動的名）。
- 注意: ウインドウ↔アプリの同定と複数ウインドウの対応付けが難所。座標は必ず `ScreenGeometry` 経由＋テスト。

- 検証: 代表的な作業セット（例: エディタ+ブラウザ+ターミナル）を保存→崩す→1キーで復元（人間ゲート: 実機）。

...
多言語化: 開発基準言語は英語。ユーザー向け文字列は String Catalog（`Sash/Resources/Localizable.xcstrings`）。正式対応は en(base)/ja、ko は任意。
...
### v1.0 に入れる（=必要な機能）
1. **キーボード配置**（MVP済み）: 左右上下の半分、四隅、1/3・2/3、最大化、中央寄せ。
...
### バックログ（v1.1 以降）
- **名前付きレイアウト保存/復元**（Moom 的。複数アプリのウインドウ配置を記録して一括復元）。
...
- **多言語**: 全文言を英語ベースで書き String Catalog に載せる。ja を正式対応、ko は任意。RTL に備え leading/trailing で組む。

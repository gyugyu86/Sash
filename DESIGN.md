...
多言語化: 開発基準言語は英語。ユーザー向け文字列は String Catalog（`Sash/Resources/Localizable.xcstrings`）。正式対応は en(base)/ja/ko/zh-Hans、アプリ内で言語切替可能（`LanguageManager`）。
...
4. **ディスプレイ間移動**: 最前面ウインドウを次/前のディスプレイへ移動（比率リサイズ）。さらに**特定ディスプレイへ直接移動**（最大6画面・⌃⌥⌘1/2/3、4–6は任意割当）。
5. ~~ドラッグスナップ~~ → **v1.0 では見送り**（macOS 標準タイルと重複）。バックログ参照。
...
10. **アプリ内 言語切替**: System/English/日本語/한국어/简体中文 をシステム言語を変えずに即時切替（`LanguageManager`）。OS リロード無し。en/ja/ko/zh-Hans 全文字列翻訳済み。
11. **自分のリソース表示**: About タブに Sash 自身のメモリ/CPU をライブ表示（`ProcessStats`、公開 Mach API）。「軽い」ことの可視化。
...
### バックログ（v1.1 以降）
- **名前付きレイアウト保存/復元**（Moom 的・キーボード中心）。当初 v1.1 旗艦として検討したが**取り下げ**（設定もの色が強く軽量路線と合わない）。
- **monitor memory（ディスプレイ構成ごとの配置記憶・自動復元）**: 実装したが基本動作が不安定＋**macOS の Spaces 制約**（他 Space のウインドウを公開APIで扱えない＝Stay/Display Maid と同じ壁）で**除去**。きちんと作り直すなら別途要設計（コードは git 履歴に残置）。

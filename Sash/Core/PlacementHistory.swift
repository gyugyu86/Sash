...
/// ウインドウごとに「配置前のフレーム」を保持し、Restore で復元できるようにする。
///
...
final class PlacementHistory {
    ...
    func recordBeforePlacement(currentFrame: CGRect, for id: CGWindowID) {
        ...
        entries[id] = Entry(restoreFrame: currentFrame, lastAppliedFrame: existing?.lastAppliedFrame)
    }

    /// 配置を適用した直後に呼ぶ。実際に適用されたフレームを記録する（最小サイズ制約対策）。
    func recordApplied(frame: CGRect, for id: CGWindowID) {

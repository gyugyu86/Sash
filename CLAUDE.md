...
- **製品方針: B＝他人にも使わせるプロダクト**を狙う。差別化の旗艦は **Phase 8（名前付きレイアウト）** ＝ v1.1。理由: 現状の配置機能は Rectangle(無料/OSS) と macOS 標準タイルにほぼ内包され差別化が無いため。
- 次は **リリース準備**（README/CHANGELOG/Homebrew cask/署名・公証。大半が人間ゲート。ライセンスは MIT 確定）→ その後 **Phase 8**。

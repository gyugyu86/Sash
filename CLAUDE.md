...
- **v1.1.1 公開済み**: 設定ウインドウのレイアウト調整のみ（正方形 700×700 / Shortcuts を2列・名前左＋録画欄右＋中央区切り線 / About を縦中央寄せ）。機能追加なし。notarize→GitHub Release→cask（本体＋tap）まで完了。`MARKETING_VERSION` は `1.1.1`。
- **残タスク（コード外・人間ゲート）**: ①宣伝用デモ GIF の作成（Kap で手動。空 TextEdit を並べ Sash 配置を実演→GIF。computer-use 自動化は TextEdit 起動時のオープンダイアログ＋デュアルモニタで難航したため手動に切替）。②note 記事 / X 投稿（日英）/ タグの公開（文面ドラフトは前チャットにあり、リポ未保存）。GIF ができたら README・note への挿入を手伝う。

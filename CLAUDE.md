...
- **v1.1（ShiftIt 参考の小機能バッチ）**: 簡体中文（zh-Hans、`AppLanguage.zhHans`）/ メニューバーアイコン非表示トグル（`showMenuBarIcon`＋reopen→Settings）/ 任意・既定OFF の更新確認（`UpdateChecker`、GitHub からバージョン番号のみ取得）/ 「問題を報告」メニュー。`feat/v1.1`。リリースは notarize→GitHub Release→cask 更新で実施。
- ③デバッグログ・⑥ズーム/全画面は ShiftIt にあるが**不採用**（軽量路線・低価値のため）。
